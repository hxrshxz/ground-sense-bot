...
export const RainfallImpactCard: React.FC<Props> = ({
  ...
  const enhancedSources = sources
    .map((s) => ({
      ...s,
      contributionScore:
        s.value * 0.4 + s.efficiency * 10 + (100 - s.seasonalVariation) * 0.2,
    }))
      ...
      <Card className="w-full overflow-hidden bg-white shadow-lg">
        ...
        <CardContent className="space-y-8">
          <div className="text-xs text-slate-600 leading-relaxed bg-sky-50/40 border border-sky-100 rounded-md p-3">
            Recharge efficiency = (annual modeled recharge / cumulative
            rainfall). Monsoon concentration indicates vulnerability to
            intra-seasonal variability. Pathway ranking incorporates efficiency,
            stability, and seasonal dispersion to guide adaptive capture
            interventions.
          </div>
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1 text-[10px] font-semibold">
                      ...
                      <span className="text-sky-600">{source.value}%</span>
                    </div>
                  </div>
                  <div className="mb-3">
                    ...
                    <div className="text-xs text-slate-600">
                      Efficiency: {source.efficiency}/5 · Variation: ±
                      {source.seasonalVariation}%
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div>
                      ...
                      <span className="ml-1 font-semibold">
                        {Math.round((100 - source.seasonalVariation) / 20)}/5
                      </span>
                    </div>
                  </div>
                      ...
                      animate={{
                        width: `${
                          (source.contributionScore /
                            Math.max(
                              ...enhancedSources.map((s) => s.contributionScore)
                            )) *
                          100
                        }%`,
                      }}
          ...
          <div
            ...
            role="tablist"
            aria-label="Rainfall analysis views"
          >
            {[
              {
                key: "pathways",
                label: "Recharge Pathways",
                icon: PieChartIcon,
              },
                    ...
                    <ResponsiveContainer width="100%" height={380}>
                      <PieChart>
                        ...
                        <Pie
                          data={sources.map((s) => ({
                            ...s,
                            effWeighted: s.value * (s.efficiency / 5),
                          }))}
                          cx="50%"
                          cy="50%"
                          outerRadius={70}
                        ...
                        <Pie
                          data={sources.map((s) => ({
                            ...s,
                            stableScore: 100 - s.seasonalVariation,
                          }))}
                          cx="50%"
                          cy="50%"
                          outerRadius={50}
                            ...
                            return (
                              <div className="rounded-md border bg-white/80 backdrop-blur-xl shadow-xl px-4 py-3 text-[11px] text-slate-700 min-w-[170px]">
                                ...
                                <div className="grid grid-cols-2 gap-x-2 gap-y-1 mt-1">
                                  ...
                                  <span>{data.efficiency}/5</span>
                                  <span className="text-amber-600 font-medium">
                                    Variation
                                  </span>
                                  <span>±{data.seasonalVariation}%</span>
                                </div>
                              </div>
                            );
                          }}
                        ...
                        <ReLegend
                          ...
                          height={40}
                          formatter={(value) => (
                            <span className="text-[11px] tracking-wide text-slate-600">
                              {value}
                            </span>
                          )}
                        />
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-lg border border-emerald-200 bg-emerald-50/50">
                      ...
                      <div className="text-sm text-emerald-600">
                        of rainfall converted to recharge
                      </div>
                    </div>
                    <div className="p-4 rounded-lg border border-sky-200 bg-sky-50/50">
                      ...
                      <div className="text-sm text-sky-600">
                        of annual recharge from monsoon
                      </div>
                    </div>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-6 border border-slate-200/70 relative overflow-hidden">
                    ...
                    <ResponsiveContainer width="100%" height={240}>
                      <BarChart
                        data={monthlyData.map((m) => ({
                          ...m,
                          efficiency:
                            Math.round((m.recharge / m.rainfall) * 100) || 0,
                        }))}
                      >
                        <CartesianGrid
                          strokeDasharray="3 3"
          ...
          <div className="bg-gradient-to-r from-sky-50 to-blue-50 border border-sky-200/80 rounded-lg p-4">
            <p className="text-sm text-slate-700 leading-relaxed">
              ...
              recharge concentrated in monsoon window; peak conversion
              efficiency in{" "}
              {
                monthlyData.reduce((max, m) =>
                  m.recharge > max.recharge ? m : max
                ).month
              }
              . Dominant pathway: {enhancedSources[0].label} (
              {enhancedSources[0].value}%, efficiency{" "}
              {enhancedSources[0].efficiency}/5). Stress {stressIndex}% reflects{" "}
              {stressIndex > 200
                ? "acute overdraft – urgent demand management required"
                : stressIndex > 150
                ? "critical stress – targeted augmentation + moderation"
                : "emerging pressures – monitor & optimize allocation"}
              .
            </p>
          </div>
