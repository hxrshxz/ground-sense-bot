                    ...
                    <div className="h-[340px] relative">
                      <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart
                          ...
                          onMouseMove={(e: any) => {
                            if (e?.activeLabel)
                              setHoverYear(e.activeLabel as number);
                          }}
                      ...
                      {hoverYear !== null && (
                        <div className="pointer-events-none absolute inset-0">
                          {(() => {
                            const idx = state.timeSeries.findIndex(
                              (d) => d.year === hoverYear
                            );
                            if (idx === -1) return null;
                            return (
                              <div
                                className="absolute top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-slate-400/60 to-transparent"
                                style={{
                                  left: `${
                                    (idx / (state.timeSeries.length - 1)) * 100
                                  }%`,
                                }}
                              />
                            );
                        ...
                        {["extraction", "recharge", "net"].map((key) => {
                          ...
                          return (
                            <span
                              ...
                              className={`px-2 py-1 rounded-md cursor-pointer select-none transition-colors border ${
                                inactive
                                  ? "text-slate-400 line-through border-slate-200"
                                  : "text-slate-700 hover:text-slate-900 border-slate-300 bg-white/60 backdrop-blur"
                              } shadow-sm`}
                            >
                              {labels[key]}
                            </span>
