...
const AnnualTrendsChart = ({ data }: { data: any }) => {
              ...
              <div className="mt-3 pt-2 border-t border-gray-200">
                <div className="text-xs text-slate-500">
                  <div className="flex justify-between mb-1">
                    ...
                    <span
                      className={`font-semibold ${
                        yearData.extraction - yearData.recharge < 0
                          ? "text-green-600"
                          : "text-red-600"
                      }`}
                    >
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-semibold ${
                        yearData.sustainability_index > 75
                          ? "text-green-600"
                          : yearData.sustainability_index > 60
                          ? "text-yellow-600"
                          : "text-red-600"
                      }`}
                    >
  ...
  const indicators = useMemo(() => {
    ...
    return {
      ...
      sustainability_index: pct(
        first.sustainability_index,
  ...
  const toggleMetric = (key: string) =>
    setMetricsActive((prev) => ({ ...prev, [key]: !prev[key] }));
  const activeHoverData = hoverYear
    ? trendsData.find((d) => d.year === hoverYear)
    : null;
    ...
    <div className="relative overflow-hidden w-full bg-white/80 p-6 rounded-2xl border my-4 backdrop-blur-md shadow-xl font-sans">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mb-4 ml-2">
          Multi-year Analysis of Extraction, Recharge & Sustainability
          (2019-2025)
        </p>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 mb-4 p-2 bg-slate-100 rounded-2xl flex-wrap">
        {[
          ...
          },
          {
            ...
            off: "text-orange-700 hover:bg-orange-100",
          },
          {
          ...
          <button
            ...
            className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 shadow-sm ${
              metricsActive[b.key] ? b.on + " shadow" : b.off + " opacity-70"
            } relative`}
          >
            {b.label}
            <span
              className={`ml-2 inline-block w-2 h-2 rounded-full ${
                metricsActive[b.key] ? "bg-white" : "bg-slate-400"
              }`}
            ></span>
          </button>
        ))}
          ...
          <ComposedChart
            ...
            onMouseMove={(s: any) => {
              if (s?.activePayload?.length)
                setHoverYear(s.activePayload[0].payload.year);
            }}
          ...
          >
            ...
            {metricsActive.decline_rate && (
              <Line
                yAxisId="right"
                type="natural"
                dataKey="decline_rate"
                name="Water Level Decline Rate"
                stroke="#f97316"
                strokeWidth={2.5}
                strokeDasharray="6 4"
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
        <IndicatorCard
          ...
          period={`${trendsData[0].year}-${
            trendsData[trendsData.length - 1].year
          }`}
          color="red"
        />
        <IndicatorCard
        ...
        <IndicatorCard
          ...
          icon="🎯"
          value={`${indicators.sustainability_index}%`}
          period={`${trendsData[0].year}-${
            trendsData[trendsData.length - 1].year
          }`}
          color="purple"
        ...
        <div className="mt-4 p-4 rounded-xl border bg-gradient-to-br from-slate-50 to-white shadow-inner grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
          <div className="col-span-2 md:col-span-4 text-slate-700 font-semibold flex items-center gap-2">
            ...
            <span className="text-base text-slate-900">
              {activeHoverData.year}
            </span>
          </div>
          <HoverMetric
            label="Extraction"
            value={activeHoverData.extraction + " HAM"}
            color="text-red-600"
          />
          <HoverMetric
            label="Recharge"
            value={activeHoverData.recharge + " HAM"}
            color="text-green-600"
          />
          <HoverMetric
            label="Decline Rate"
            value={activeHoverData.decline_rate + " m/yr"}
            color="text-orange-600"
          />
          <HoverMetric
            label="Sustainability"
            value={activeHoverData.sustainability_index + "%"}
          ...
          <div className="col-span-2 md:col-span-4 text-[10px] text-slate-500 border-t pt-2 flex flex-wrap gap-4">
            <span>
              ...
              <strong
                className={
                  activeHoverData.extraction > activeHoverData.recharge
                    ? "text-red-600"
                    : "text-green-600"
                }
              >
            ...
            <span>
              ...
              <strong
                className={
                  activeHoverData.sustainability_index > 75
                    ? "text-green-600"
                    : activeHoverData.sustainability_index > 60
                    ? "text-yellow-600"
                    : "text-red-600"
                }
              >
...
const InsightGrid = ({ data }: { data: any[] }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-slate-600">
      <div>
        • Extraction rising ~{annual(first.extraction, last.extraction)} HAM /
        yr
      </div>
      <div>
        • Recharge falling ~
        {Math.abs(parseFloat(annual(first.recharge, last.recharge)))} HAM / yr
      </div>
      <div>
        • Decline rate accelerating (+
        {annual(first.decline_rate, last.decline_rate)} m/yr² proxy)
      </div>
      <div>
        • Sustainability dropping{" "}
        {Math.abs(
          parseFloat(
            annual(first.sustainability_index, last.sustainability_index)
          )
        )}{" "}
        pts / yr
      </div>
