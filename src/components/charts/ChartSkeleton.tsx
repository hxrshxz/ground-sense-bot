...
export const ChartSkeleton: React.FC<ChartSkeletonProps> = ({ height = 260, variant = 'line', className = '' }) => {
  const base = 'rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-slate-100 overflow-hidden relative';
  return (
    <div className={`${base} ${className}`} style={{ height }} aria-label="Loading chart" role="status">
        ...
        <div className="flex-1 relative">
          {variant === 'line' && (
            <div className="h-full flex items-end gap-2">
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className={`flex-1 ${shimmer} bg-slate-200/70 rounded-t`} style={{ height: `${20 + (i%5)*10}%` }} />
              ))}
          ...
          {variant === 'donut' && (
            <div className="h-full flex items-center justify-center">
              <div className={`relative ${shimmer} w-40 h-40 rounded-full bg-gradient-to-br from-slate-200 to-slate-100`}> 
          ...
          {variant === 'bar' && (
            <div className="h-full flex items-end gap-2">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className={`flex-1 ${shimmer} rounded bg-slate-200/70`} style={{ height: `${30 + (i%4)*12}%` }} />
              ))}
            </div>
          )}
          {variant === 'radar' && (
            <div className="h-full flex items-center justify-center">
              <div className="relative w-48 h-48">
                ...
                <div className="absolute inset-0 origin-center translate-x-1/2 translate-y-1/2 -left-1/2 -top-1/2 w-px h-full bg-slate-200/60 -rotate-45" />
                <div className={`absolute inset-0 m-auto w-20 h-20 bg-slate-200/70 ${shimmer} clip-path-polygon`} />
              </div>
            </div>
          )}
          {variant === 'kpi' && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 h-full">
              {Array.from({ length: 4 }).map((_, i)=> (
                <div key={i} className="relative rounded-xl border border-slate-200 bg-white/60 p-3">
