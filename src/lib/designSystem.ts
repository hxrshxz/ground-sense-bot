...
export const FOUR_KEY_ATTRIBUTES = {
  ...
  STAGE: {
    ...
    unit: '%',
    icon: '📉',
    description: 'Extraction as percentage of available resources (Extraction/Extractable × 100)',
  },
  CATEGORY: {
    id: 'category',
 ...
 */
export function getCategoryFromStage(stage: number): keyof typeof CATEGORY_COLORS {
 ...
 */
export function formatWithUnit(num: number | undefined | null, unit: string): string {
    ...
    series: [
      {
        ...
        type: 'bar',
        data: [
          { value: data.extractable, itemStyle: { color: ATTRIBUTE_COLORS.extractable.primary } },
          { value: data.extraction, itemStyle: { color: ATTRIBUTE_COLORS.extraction.primary } },
        ],
      },
    ],
  };
...
export function createCategoryDistributionChart(data: {
  ...
  return {
    ...
    series: [{
      ...
      radius: ['40%', '70%'],
      data: [
        { value: data.safe, name: 'Safe', itemStyle: { color: CATEGORY_COLORS.safe.primary } },
        { value: data.semiCritical, name: 'Semi-Critical', itemStyle: { color: CATEGORY_COLORS.semiCritical.primary } },
        { value: data.critical, name: 'Critical', itemStyle: { color: CATEGORY_COLORS.critical.primary } },
        { value: data.overExploited, name: 'Over-Exploited', itemStyle: { color: CATEGORY_COLORS.overExploited.primary } },
      ].filter(d => d.value > 0),
    }],
  };
}
 ...
 */
export function createStageRankingChart(data: Array<{
  name: string;
  stage: number;
  category: string;
}>) {
